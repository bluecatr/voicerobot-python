...
def check_network_connection(server="www.rtongcloud.com"):
    """
    Checks if main can connect a network server.
...
def get_pip_requirements(fname=os.path.join(configuration.DATA_PATH,
                                            'requirements.txt')):
    """
    ...
    if os.access(fname, os.R_OK):
        reqs = list(pip.req.parse_requirements(fname, session=False))
...
def run():
    ...
    for fname in [os.path.join(configuration.CONFIG_PATH, "g014b2b","g014b2b.fst")]:
