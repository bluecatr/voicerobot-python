...
while [ $3 ]; do
	...
	ls $1 | while read line; do
		let CURRENT+=1
		$cur_dir/silk/decoder "$1/$line" "$2/$line.pcm" -Fs_API 16000 > /dev/null 2>&1
		...
		ffmpeg -y -f s16le -ar 16000 -ac 1 -i "$2/$line.pcm" "$2/${line%.*}.$3" > /dev/null 2>&1 &
		ffmpeg_pid=$!
		while kill -0 "$ffmpeg_pid"; do sleep 1; done > /dev/null 2>&1
		rm "$2/$line.pcm"
...
$cur_dir/silk/decoder "$1" "$1.pcm" -Fs_API 16000 > /dev/null 2>&1
if [ ! -f "$1.pcm" ]; then
